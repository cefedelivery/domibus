...
public class BackendJMSImpl extends AbstractBackendConnector<MapMessage, MapMessage> {
    ...
    private final static String UMDS_REJECTECT_TEMPLATE =  "<?xml version='1.0' encoding='UTF-8'?>\n" +
            "<message_id>\n" +
            " $messId\n" +
            "</message_id>\n" +
    ...
    public void deliverMessage(final String messageId) {
        ...
        try {
            ...
            String message = HAPPY_FLOW_MESSAGE_TEMPLATE.replace("$messId", messageId);
            ...
            String message = UMDS_REJECTECT_TEMPLATE.replaceFirst("$messId", messageId);
            submission.getPayloads().clear();
            submission.addPayload(getPayload(message, MediaType.TEXT_XML));
            try {
                messageSubmitter.submit(submission, getName());
