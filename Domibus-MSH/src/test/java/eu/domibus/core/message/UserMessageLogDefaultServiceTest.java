...
public class UserMessageLogDefaultServiceTest {
    ...
    public void testSaveWithSubtype(boolean valid) {
        final String messageId = "1";
        final String messageStatus = MessageStatus.SEND_ENQUEUED.toString();
        final String notificationStatus = NotificationStatus.NOTIFIED.toString();
        final String mshRole = MSHRole.SENDING.toString();
        ...
        final String backendName = "JMS";
        final String endpoint = "http://localhost";
        final String messageSubtype = "TEST";

        userMessageLogDefaultService.save(messageId, messageStatus, notificationStatus, mshRole, maxAttempts, mpc, backendName, endpoint, valid ? messageSubtype : null);
        ...
        new Verifications() {{
            ...
            Assert.assertEquals(backendName, userMessageLog.getBackend());
            Assert.assertEquals(endpoint, userMessageLog.getEndpoint());
            Assert.assertEquals(valid ? MessageSubtype.TEST : null, userMessageLog.getMessageSubtype());
        }};
    }

    @Test
    public void testSaveWithValidSubtype() {
        testSaveWithSubtype(true);
    }

    @Test
    public void testSaveWithInvalidSubtype() {
        testSaveWithSubtype(false);
    }
