 ...
 */
public class WSMessageTransformer implements MessageSubmissionTransformer<Messaging>, MessageRetrievalTransformer<UserMessage> {
    ...
    private void generateCollaborationInfo(final Submission submission, final UserMessage result) {
        ...
        if (!submission.getConversationId().isEmpty()) {
    ...
    private void generateMessageInfo(final Submission submission, final UserMessage result) {
        ...
        if (!submission.getMessageId().isEmpty()) {
    ...
    public Submission transformToSubmission(Messaging messageData) {
        UserMessage userMessage = messageData.getUserMessage();

        final Submission result = new Submission();

        final CollaborationInfo collaborationInfo = userMessage.getCollaborationInfo();
        result.setAction(collaborationInfo.getAction());
        result.setService(userMessage.getCollaborationInfo().getService().getValue());
        result.setServiceType(userMessage.getCollaborationInfo().getService().getType());
        ...
        result.setMessageId(userMessage.getMessageInfo().getMessageId());
        result.setRefToMessageId(userMessage.getMessageInfo().getRefToMessageId());
        ...
        result.setFromRole(userMessage.getPartyInfo().getFrom().getRole());
        result.setToRole(userMessage.getPartyInfo().getTo().getRole());
